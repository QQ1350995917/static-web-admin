  ...
  <el-container>
    ...
    <el-main>
      <el-table ref="mainTableList" :data="mainTableDataList" row-key="id" style="width: 100%" stripe
        ...
        <el-table-column label="pid" prop="pid"  width="120" sortable='custom' fixed>
          <template slot-scope="scope">
...
//            }
    ...
    methods: {
      ...
      requestForMainTableList() {
        this.$store.dispatch('accessAdminRole/list', this.query)
        .then((response) => {
      ...
      requestForUserEnable(ids, successCallback){
        this.$store.dispatch('accessAdminRole/enable', ids)
        .then(successCallback()).catch((error) => {
          this.$message.error(error);
      })
      },
      requestForUserDisable(ids, successCallback){
        this.$store.dispatch('accessAdminRole/disable', ids)
        .then(successCallback()).catch((error) => {
          this.$message.error(error);
      })
      },
      ...
      requestForDelete(ids){
        this.$store.dispatch('accessAdminRole/del', ids)
        .then(() => {
          this.$message({
          message: '删除成功',
          type: 'success'
      ...
      requestForCreateMember(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$store.dispatch('accessAdminRole/create', this.createDialogForm)
            .then((response) => {
              if (response.meta.code == 200) {
                this.$message({
                  message: '创建成功',
      ...
      requestForEditMember(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$store.dispatch('accessAdminRole/edit', this.createDialogForm)
            .then((response) => {
              if (response.meta.code == 200) {
                this.$message({
                  message: '修改成功',
